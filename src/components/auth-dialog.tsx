...
export function AuthDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function googleLogin() { setLoading(true); setNotice(null); const client = getSupabaseBrowserClient(); if (!client) { setLoading(false); setNotice({ kind: "error", text: "Member access is being configured. Please join us on Telegram for announcements." }); return; } const { error } = await client.auth.signInWithOAuth({ provider: "google", options: { redirectTo: `${location.origin}/auth/callback` } }); if (error) { setLoading(false); setNotice({ kind: "error", text: error.message }); } }
