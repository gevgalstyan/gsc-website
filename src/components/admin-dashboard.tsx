...
type NotificationRow = { id: string; kind: string; title: string; body: string; meetup_id: string | null; booking_id: string | null; target_url: string | null; read_at: string | null; created_at: string };
...
export function AdminDashboard({ initial }: { initial: AdminData }) {
  ...
  const [announcement, setAnnouncement] = useState({ title: "", body: "", target: "" });
  ...
  useEffect(() => {
    const refreshLiveData = async () => {
      ...
      const [meetupResult, bookingResult, attendanceResult, notificationResult] = await Promise.all([
        ...
        client.from("notifications").select("id,kind,title,body,meetup_id,booking_id,target_url,read_at,created_at").eq("user_id", initial.currentUserId).order("created_at", { ascending: false }).limit(20),
      ]);
  ...
  async function publishAnnouncement(event: FormEvent<HTMLFormElement>) {
    ...
    if (!announcement.title.trim() || !announcement.body.trim()) return message("Add an announcement title and message.");
    setBusy(true);
    const { error } = await getSupabaseBrowserClient()!.rpc("admin_publish_announcement", { p_title: announcement.title.trim(), p_body: announcement.body.trim(), p_target_url: announcement.target.trim() || null });
    setBusy(false);
    if (error) return message("The announcement could not be sent. Use an internal link or leave it blank.");
    setAnnouncement({ title: "", body: "", target: "" });
    message("Announcement sent to current members.");
  }
