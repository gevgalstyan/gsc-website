...
export function AdminDashboard({ initial }: { initial: AdminData }) {
  ...
  async function saveMeetup(event: FormEvent<HTMLFormElement>) {
    ...
    const client = getSupabaseBrowserClient();
    const payload = {
      title: meetupForm.title.trim(), description: meetupForm.description.trim(), starts_at: new Date(meetupForm.starts_at).toISOString(), ends_at: new Date(meetupForm.ends_at).toISOString(), timezone: meetupForm.timezone.trim(), location_name: meetupForm.location_name.trim(), address: meetupForm.address.trim() || null, capacity: Number(meetupForm.capacity), price_minor: Math.round(Number(meetupForm.price) * 100), currency: meetupForm.currency.toUpperCase(), category: meetupForm.category.trim() || "Conversation", image_url: meetupForm.image_url.trim() || null, status: meetupForm.status, is_public: meetupForm.status !== "draft", booking_opens_at: meetupForm.booking_opens_at ? new Date(meetupForm.booking_opens_at).toISOString() : null, booking_closes_at: meetupForm.booking_closes_at ? new Date(meetupForm.booking_closes_at).toISOString() : null,
    };
  ...
  async function saveManagedQuestion(event: FormEvent<HTMLFormElement>) {
    ...
    const { data: row, error } = await getSupabaseBrowserClient()!.from("managed_questions").insert(payload).select("id,prompt,translation,category,difficulty,is_published").single();
    setBusy(false);
    if (error || !row) return message("Question could not be added. Apply the platform migration first if this is a new database.");
    setManagedQuestions((rows) => [row, ...rows]); event.currentTarget.reset(); message("Question added to the managed library.");
  }
  ...
  async function deleteManagedQuestion(id: string) {
    ...
    setManagedQuestions((rows) => rows.filter((row) => row.id !== id)); message("Question deleted.");
  }

  async function saveContent(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy(true); const data = new FormData(event.currentTarget);
    const key = String(data.get("key") ?? "").trim(); const value = String(data.get("value") ?? "").trim();
    const { data: row, error } = await getSupabaseBrowserClient()!.from("site_content").upsert({ key, value, is_public: true }).select("key,value,is_public,updated_at").single(); setBusy(false);
    if (error || !row) return message("Content could not be saved.");
    setContent((rows) => [row, ...rows.filter((item) => item.key !== row.key)]); message("Content saved.");
  }
