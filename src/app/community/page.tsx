...
export default async function CommunityPage() {
  ...
  return <PublicPageShell eyebrow="Community" title={content["community.hero.title"] || "A reason to keep English in your life"} intro={content["community.intro"] || "Galstyan’s Speaking Club is a local Sergiev Posad community for conversation, confidence, and the people you meet along the way."} breadcrumbLabel="Community" breadcrumbPath="/community">
    ...
    <section className="section public-section public-cta-panel"><Users /><div><span className="eyebrow">Join the local community</span><h2>Your next good conversation can start <em>here.</em></h2><p>{viewer.role === "loggedOut" ? "Follow meetup announcements on Telegram, then create a profile when you want bookings and progress in one place." : "Follow meetup announcements on Telegram, and use your dashboard to keep bookings, attendance, and progress together."}</p><div className="public-actions"><a className="button button-primary" href="https://t.me/GalstyansSpeakingClub" target="_blank" rel="noreferrer">Join Telegram <ArrowRight /></a><Link className="button button-outline-dark" href={viewer.role === "loggedOut" ? "/membership" : "/account"}>{viewer.role === "loggedOut" ? "See membership" : "Open dashboard"} <ArrowRight /></Link></div></div></section>
  </PublicPageShell>;
}
