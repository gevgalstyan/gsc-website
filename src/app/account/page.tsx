...
export default async function AccountPage() {
  ...
  const [profileResult, roleResult, attendanceResult, bookingsResult, rewardsResult, specialResult, progressResult, favoritesResult] = await Promise.all([
    supabase.from("profiles").select("display_name,avatar_path,avatar_url,telegram_username,created_at").eq("id", userId).single(),
  ...
  const profile = profileResult.data ?? { display_name: null, avatar_path: null, avatar_url: null, telegram_username: null, created_at: new Date().toISOString() };
  ...
  return <main className="dashboard-shell">
    <header className="dashboard-topbar">
      ...
      <nav aria-label="Member navigation"><Link href="/#questions">Questions</Link><Link href="/#meetups">Meetups</Link><Link href="/#community">Community</Link>{roleResult.data?.role === "admin" && <Link href="/admin">Admin</Link>}</nav>
